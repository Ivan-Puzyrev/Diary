...
object RandomTasksGenerator {
    ...
    private fun getRandomTasks(): LinkedList<Pair<String, String>> {
        ...
        randomTasksList.add(Pair("Добавить возможность изменения темы", "Необходимо добавить в приложение функциональность для переключения между светлой и темной темами. Выбор темы должен сохраняться даже после закрытия и перезапуска приложения."))
